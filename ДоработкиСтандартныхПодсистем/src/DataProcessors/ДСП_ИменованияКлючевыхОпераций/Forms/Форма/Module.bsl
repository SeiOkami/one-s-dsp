...
Процедура Команда_ВыполнитьДействие(Команда)
	...
	Если ЗначениеЗаполнено(Объект.КлючевыеОперации) Тогда
		...
		ВыполнитьДействие();
		ОповеститьОбИзменении(Тип("СправочникСсылка.КлючевыеОперации"));
		ЗаполнитьДанные();
		
	Иначе
		
		ПоказатьПредупреждение(, "Необходимо сначала заполнить ключевые операции к переименованию!");
...
&НаКлиенте
Процедура ИзменитьФлажки(НовоеЗначение, Сколько = Неопределено)
	
	НомерСтроки = 0;
	Для Каждого Строка Из Объект.КлючевыеОперации Цикл
		
		НомерСтроки = НомерСтроки + 1;
		Если Сколько <> Неопределено И НомерСтроки > Сколько Тогда
			Прервать;
		КонецЕсли;
